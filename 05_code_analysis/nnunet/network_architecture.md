...
def get_network_from_plans(plans, dataset_json, configuration, num_input_channels):
    ...
    features_per_stage = compute_features(
        plan["UNet_base_num_features"], plan["unet_max_num_features"], num_stages
    )
...
total_loss = sum(
    w * loss(pred, downsampled_target) for w, pred in zip(weights, seg_outputs)
)
```
